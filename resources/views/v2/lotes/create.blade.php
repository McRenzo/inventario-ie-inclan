    ...
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-1 text-sm text-slate-500">
                Registra bienes o materiales controlados por cantidad.
            </p>
        </div>
    ...
    >
        ...
        <div class="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm">
            <div class="mb-5">
                ...
                <p class="mt-1 text-xs text-slate-400">
                    Selecciona una ficha de tipo lote o consumible.
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                ...
                <div class="md:col-span-2">
                    ...
                    >
                        ...
                        @foreach ($bienes as $bien)
                            <option
                                value="{{ $bien->id }}"
                                @selected(
                                    old('bien_id', $bienSeleccionado) == $bien->id
                                )
                            >
                                {{ $bien->nombre }}
                    ...
                    @if ($bienes->isEmpty())
                        <p class="mt-2 text-xs font-semibold text-amber-600">
                            Primero debes crear una ficha con tipo de control lote o consumible.
                        </p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <input
                        type="text"
                        name="unidad_medida"
                        value="{{ old('unidad_medida', 'unidad') }}"
                        required
                        placeholder="unidad, caja, paquete, metro..."
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                    >
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2">
                    ...
                    >
                        ...
                        @foreach ($estadosConservacion as $estado)
                            <option
                                value="{{ $estado->id }}"
                                @selected(
                                    old('estado_conservacion_id') == $estado->id
                                )
                            >
                                {{ $estado->nombre }}
                            </option>
                    ...
                    >
                        ...
                        @foreach ($estadosOperatividad as $estado)
                            <option
                                value="{{ $estado->id }}"
                                @selected(
                                    old('estado_operatividad_id') == $estado->id
                                )
                            >
                                {{ $estado->nombre }}
                            </option>
                ...
                <div>
                    ...
                    >
                        <option
                            value="disponible"
                            @selected(old('situacion', 'disponible') === 'disponible')
                        >
                            Disponible
                        </option>
                        ...
                        <option
                            value="asignado"
                            @selected(old('situacion') === 'asignado')
                        >
                            Asignado
                        </option>
                    </select>
        ...
        <div class="rounded-3xl border border-slate-200/70 bg-white p-6 shadow-sm">
            <div class="mb-5">
                ...
                <p class="mt-1 text-xs text-slate-400">
                    Completa esta sección solo si el lote está asignado a una persona.
                </p>
            </div>
            ...
            <div class="grid grid-cols-1 gap-5 md:grid-cols-2 xl:grid-cols-3">
                ...
                <div>
                    ...
                    <input
                        ...
                        placeholder="Factura, boleta, acta..."
                        class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm outline-none transition focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-500/10"
                    >
                </div>
