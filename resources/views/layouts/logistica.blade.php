...
<body class="bg-[#f4f7fa] antialiased text-slate-800 select-none">
    ...
    <div class="flex min-h-screen p-0 lg:p-4 gap-4">
        ...
        <aside class="hidden lg:flex w-68 bg-white/80 text-slate-600 flex-col justify-between fixed h-[calc(100vh-2rem)] z-20 rounded-3xl shadow-[0_10px_30px_-10px_rgba(0,30,80,0.05)] border border-slate-200/60 backdrop-blur-xl">
            <div>
                <div class="p-6 flex items-center space-x-3.5 border-b border-slate-100 bg-slate-50/50 rounded-t-3xl">
                    <div class="bg-gradient-to-tr from-blue-600 to-blue-500 p-2.5 rounded-2xl shadow-md shadow-blue-500/20">
                    ...
                    <div>
                        ...
                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-md text-[9px] font-bold bg-blue-600/10 text-blue-600 border border-blue-500/10 uppercase tracking-widest mt-1.5">Sistema QR</span>
                    </div>
                </div>

                <nav class="px-4 py-4 space-y-6 font-medium text-xs">
                    <div class="space-y-1">
                        <p class="px-3 pb-1 text-[10px] font-bold text-slate-400 uppercase tracking-widest">Monitoreo</p>
                        
                        <a href="{{ route('dashboard') }}" class="flex items-center justify-between px-3 py-2.5 rounded-xl transition-all duration-200 group {{ Request::is('dashboard') ? 'bg-blue-600 text-white font-bold shadow-lg shadow-blue-600/20' : 'text-slate-600 hover:bg-slate-100/80 hover:text-slate-900' }}">
                            <div class="flex items-center space-x-3">
                                <svg class="w-4 h-4 transition-transform group-hover:scale-105" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg>
                                <span>Dashboard</span>
                            </div>
                        </a>
            ...
            <div class="p-3 m-3 bg-slate-50 border border-slate-100 rounded-2xl flex items-center justify-between shadow-inner">
                <div class="flex items-center space-x-2.5 overflow-hidden">
                    <div class="w-9 h-9 rounded-xl bg-blue-600 flex items-center justify-center font-black text-white text-xs shadow-md shadow-blue-600/10 shrink-0">
                ...
                <form method="POST" action="{{ route('logout') }}" class="inline shrink-0">
                    @csrf
                    <button type="submit" class="p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition border border-transparent" title="Cerrar sesión">
        ...
        <div class="flex-1 lg:pl-72 flex flex-col min-h-screen">
            ...
            <header class="h-16 bg-white/70 border border-slate-200/60 flex items-center justify-between px-6 rounded-2xl sticky top-0 lg:top-4 z-10 shadow-[0_4px_20px_-5px_rgba(0,0,0,0.02)] backdrop-blur-md">
